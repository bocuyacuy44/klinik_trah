...
const CreateRegistration: React.FC<CreateRegistrationProps> = ({
  ...
  const ruanganOptions = [
    { value: 'poli-umum', label: 'Poli Umum', kuota: 20, terpakai: 5 },
    { value: 'poli-anak', label: 'Poli Anak', kuota: 15, terpakai: 8 },
    { value: 'poli-gigi', label: 'Poli Gigi', kuota: 10, terpakai: 3 }
  ];

  const dokterOptions = [
    { value: 'dr-kartini', label: 'dr. Sri Kartini Kussudiardjo, Sp.A' },
    { value: 'dr-ahmad', label: 'dr. Ahmad Santoso' },
    { value: 'dr-sari', label: 'dr. Sari Dewi' }
  ];
  ...
  const selectedRuangan = ruanganOptions.find(r => r.value === formData.ruangan);
  const sisaKuota = selectedRuangan ? selectedRuangan.kuota - selectedRuangan.terpakai : 0;
  const totalKuota = selectedRuangan ? selectedRuangan.kuota : 0;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.ruangan || !formData.dokter || !formData.caraBayar || !formData.penjamin) {
    ...
    try {
      setLoading(true);
      await registrationService.createRegistration(patient.id);
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => handleInputChange('caraBayar', e.target.value)}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                  required
                >
                  <option value="">Pilih Cara Bayar</option>
                  {caraBayarOptions.map(option => (
                    <option key={option.value} value={option.value}>
                      {option.label}
                    </option>
                  ))}
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">
                  Penjamin <span className="text-red-500">*</span>
                </label>
                <select
                  value={formData.penjamin}
                  onChange={(e) => handleInputChange('penjamin', e.target.value)}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                  required
                >
                  <option value="">Pilih Penjamin</option>
                  {penjaminOptions.map(option => (
                    <option key={option.value} value={option.value}>
                      {option.label}
                    </option>
                  ))}
